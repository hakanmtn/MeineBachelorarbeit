...
public record AddIndexOperation(int index, Integer element) implements CollectionOperation<Integer, Void> {
    ...
    public BoolExpr listContract(SeqExpr<IntSort> oldContent, SeqExpr<IntSort> newContent,
        ...
        IntExpr suffixLen = (IntExpr) context.mkSub(context.mkLength(oldContent), context.mkInt(index));
        SeqExpr<IntSort> suffix = context.mkExtract(oldContent, context.mkInt(index), suffixLen);
        ...
        return context.mkAnd(precondition, postcondition);
    }
